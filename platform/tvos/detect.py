...
def configure(env: "SConsEnvironment"):
    ...
    if env["simulator"]:
        env["APPLE_PLATFORM"] = "appletvsimulator"
        env.Append(ASFLAGS=["-mtargetos=tvos15.0-simulator"])
        env.Append(CCFLAGS=["-mtargetos=tvos15.0-simulator"])
        env.Append(CPPDEFINES=["TVOS_SIMULATOR"])
        env.extra_suffix = ".simulator" + env.extra_suffix
    else:
        env["APPLE_PLATFORM"] = "appletvos"
        env.Append(ASFLAGS=["-mtargetos=tvos15.0"])
        env.Append(CCFLAGS=["-mtargetos=tvos15.0"])
    detect_darwin_sdk_path(env["APPLE_PLATFORM"], env)

    if env["arch"] == "x86_64":
        if not env["simulator"]:
            print_warning("Building for tvOS with 'arch=x86_64' requires 'simulator=yes'.")
            sys.exit(255)
    ...
    # Temp fix for ABS/MAX/MIN macros in the Apple SDK blocking compilation
    ...
    # NOTE (tvOS divergence from iOS/visionOS): the tvOS Simulator DOES support Metal on
    # Apple Silicon, and tvOS has NO OpenGL ES fallback — so Metal must stay enabled for the
    # simulator too (iOS/visionOS disable Metal on their simulators and fall back to GL/none).
    if env["metal"]:
